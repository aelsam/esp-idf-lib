...
#define I2C1_SDA_IO_NUM		18
#define I2C1_SCL_IO_NUM		19
...
#define DHT12_ADDRESS			((uint8_t)0xB8)
...
SemaphoreHandle_t	DHT12_Lock;

sDht12Data Dht12DataGet(void)
{
	sDht12Data Temp;
	xSemaphoreTake(DHT12_Lock, portMAX_DELAY);
	memcpy(&Temp, &Dht12Data, sizeof(Dht12Data));
	xSemaphoreGive(DHT12_Lock);
	return Temp;
}
...
// Convert to i2cdev for thread safe calls
	...
	i2c_config_t I2cConfig = {
		...
		.sda_io_num = I2C1_SDA_IO_NUM,
		.sda_pullup_en = GPIO_PULLUP_ENABLE,
		.scl_io_num = I2C1_SCL_IO_NUM,
	...
	DHT12_Lock = xSemaphoreCreateMutex();
	if (!DHT12_Lock) {
		return -1;
	}
...
{	
	Dht12Init();
	vSemaphoreCreateBinary( DHT12_Lock );
	int Ret = 0;
	while(1) {
