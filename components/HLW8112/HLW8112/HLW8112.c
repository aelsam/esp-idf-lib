...
/*---------------------B通道-------------------------------*/
unsigned int    U16_RMSIBC_RegData;           //B通道电流系数,HLW8112硬件上没有，但寄存器存在 
unsigned int    U16_PowerPBC_RegData;         //B通道功率系数,HLW8112硬件上没有，但寄存器存在 
unsigned int    U16_EnergyBC_RegData;         //B通道电量系数,,HLW8112硬件上没有，但寄存器存在
unsigned long   U32_RMSIB_RegData;            //B通道电流寄存器有效值,HLW8112硬件上没有，但寄存器存在 
unsigned long   U32_POWERPB_RegData;          //A通道功率寄存器有效值 
...
float   U32_AC_I_B;                     //B通道电流有效值
float   U32_AC_P_B;                     //B通道有功功率
float   U32_AC_E_B;                     //B通道有功电能(量)
float   U32_AC_BACKUP_E_B;              //B通道电量备份   
float   U8_AC_PF_B;  
float   U8_Angle_B;
...
//因为K1和K2都是1，所以a/(K1*K2) = a    
  ...
  //清零 REG_ENERGY_PB_ADDR寄存器
    Uart_Write_HLW8112_Reg(REG_EMUCON2_ADDR,2,0x03ff);  //0x0001是EMUCON2的默认值，waveEn = 1,zxEn = 1，A通道电量寄存器，读后不清0，EPA_CB = 1；打开功率因素检测
      ...
      Uart_Write_HLW8112_Reg(REG_EMUCON2_ADDR,2,0x0fff);  //0x0001是EMUCON2的默认值，waveEn = 1,zxEn = 1，A通道电量寄存器，读后不清0，EPA_CB = 1；打开功率因素检测
      vTaskDelay(50 / portTICK_PERIOD_MS);
      Uart_HLW8112_WriteREG_DIS();   //关闭写8112 Reg
      vTaskDelay(50 / portTICK_PERIOD_MS);
...
//因为K1和K2都是1，所以a/(K1*K2) = a    
  ...
  //清零 REG_ENERGY_PB_ADDR寄存器
    Uart_Write_HLW8112_Reg(REG_EMUCON2_ADDR,2,0x07ff);  //0x0001是EMUCON2的默认值，waveEn = 1,zxEn = 1，A通道电量寄存器，读后不清0，EPA_CB = 1；打开功率因素检测
...
void Reset_Energy_A()
{
  ...
  //清零 REG_ENERGY_PB_ADDR寄存器
    Uart_Write_HLW8112_Reg(REG_EMUCON2_ADDR,2,0x0bff);  //0x0001是EMUCON2的默认值，waveEn = 1,zxEn = 1，A通道电量寄存器，读后不清0，EPA_CB = 1；打开功率因素检测
...
//    Uart_Write_HLW8112_Reg(REG_SYSCON_ADDR,2,0x0a04);   //开启A通道，关闭B通道，电压通道PGA = 1，电流通道PGA = 16
    Uart_Write_HLW8112_Reg(REG_SYSCON_ADDR,2,0x0f04);   //开启A通道，关闭B通道，电压通道PGA = 1，电流通道PGA = 16
    vTaskDelay(50 / portTICK_PERIOD_MS);
    
    /*------    过零输出设置-------以下代码，如果不需要可以屏蔽 ------*/
    ESP_LOGI(TAG, "Setup Counter");
//    Uart_Write_HLW8112_Reg(REG_EMUCON_ADDR,2,0x0001);   //1，使能PFA 脉冲输出和有功电能寄存器累加；
    Uart_Write_HLW8112_Reg(REG_EMUCON_ADDR,2,0x1003);   //1，使能PFA 脉冲输出和有功电能寄存器累加；

    vTaskDelay(50 / portTICK_PERIOD_MS);
//  Uart_Write_HLW8110_Reg(REG_EMUCON_ADDR,2,0x0018);   //正向和负向过零点均发生变化，ZXD0 = 1，ZXD1 = 1
    ESP_LOGI(TAG, "EMNUCON2 setup");
//    Uart_Write_HLW8112_Reg(REG_EMUCON2_ADDR,2,0x0465);  //0x0001是EMUCON2的默认值，waveEn = 1,zxEn = 1，A通道电量寄存器，读后不清0，EPA_CB = 1；打开功率因素检测
    Uart_Write_HLW8112_Reg(REG_EMUCON2_ADDR,2,0x0fff);  //0x0001是EMUCON2的默认值，waveEn = 1,zxEn = 1，A通道电量寄存器，读后不清0，EPA_CB = 1；打开功率因素检测
